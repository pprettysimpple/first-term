...
big_integer::big_integer() : sign_(false), digits_(1, 0) {}

big_integer::big_integer(int a) : sign_(a < 0 ? UINT32_MAX : 0), digits_(1, a) {}

big_integer::big_integer(uint32_t a) : sign_(0), digits_(1, a) {}

big_integer::big_integer(uint64_t a) : sign_(a < 0 ? UINT32_MAX : 0), digits_(2) {
...
big_integer &big_integer::_divide_n_1(uint32_t rhs) {
    uint64_t carry = 0;
...
big_integer &big_integer::_divide_m_n(big_integer &rhs) {
...
big_integer &big_integer::_divide_unsigned_normalized(big_integer &rhs) {
    if (rhs.digits_.size() == 1) {
        return _divide_n_1(rhs.digits_[0]);
    }
    if (digits_.size() == 2) {
    ...
    return _divide_m_n(rhs);
}

big_integer &big_integer::_divide_unsigned(big_integer &rhs) {
    if (digits_.size() < rhs.digits_.size()) {
        return *this = 0;
    }
    ...
    if (clz) {
        *this <<= clz;
        return _divide_unsigned_normalized(rhs <<= clz);
    } else {
        return _divide_unsigned_normalized(rhs);
    }
}
...
big_integer &big_integer::operator/=(big_integer const &rhs) {
    ...
    if (rhs.sign_) return *this = -(*this /= (-rhs));
    big_integer copy_rhs(rhs);
    return *this = _divide_unsigned(copy_rhs);
}
